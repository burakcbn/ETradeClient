...
export class RegisterComponent extends BaseComponent implements OnInit {
    ...
    this.frm = this.formBuilder.group({
    ...
    },
      {
        validators: (group: AbstractControl): ValidationErrors | null => {
          ...
          return passwordRepeat === password ? null : { notSame: true };
        }
      })
  }
  get component() {
  ...
  async onSubmit(user: User) {
    ...
    const createUser: CreateUser = await this.userService.create(user);

    if (createUser.succeeded)
      this.toastrService.message(createUser.message, "Başarılı", {
        messageType: ToastrMessageType.Success,
        position: ToastrPosition.TopRight
      })
    else
      this.toastrService.message(createUser.message, "Hata", {
